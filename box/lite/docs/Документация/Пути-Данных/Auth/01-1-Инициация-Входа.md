...
- Создает ссылку редиректа на страницу нашего сайта после логина через внешний сервис
- Эта ссылка берется либо из referer:
  если пользователь нажимает логин через интерфейс, то referer - адрес страницы на которой был пользователь, если пользователь тянет ручку напрямую а не через интерфейс, то берется просто адрес ручки и к нему приписывается /callback
- URL может содержать какой-то callback в query запросе, если это так, то берем его, иначе используем дефолтный на login
- Далее вызываем функцию для создания URL для OIDC, передаем туда redirectURL и id сессии для CSRF
- Возвращаем пользователю ответ в котором происходит его редирект на внешний сервис где он логинится, а потом его редирект на redirectURL
...
Что делает программа:
- Создает OIDC-client, 
- Через клиент возвращает URL, ведущий пользователя на внешнего провайдера (Google, Microsoft и т.д.).
- После логина провайдер редиректит его обратно на redirectURI, передавая код авторизации.
- Код вместе с CSRF-токеном проверяется, и приложение получает id_token и, при необходимости, access_token.
